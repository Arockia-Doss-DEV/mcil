...
class IndividualController extends Controller
{
    public function index(Request $request) 
    {
        ...
        $reinvest_query = Subscription::where('status', 6)
                                        ->where('user_id', $user_id)
                                        ->where('redemption_status', 1)
                                        ->get();

        $reinvest_investment_count = $reinvest_query->count();  

        $redemption_upcomeing_query = Subscription::where('status', 3)
                                        ->where('user_id', $user_id)
                                        ->where('redemption_status', 1)
                                        ->get();

        $redemption_upcomeing_count = $redemption_upcomeing_query->count(); 
        ...
        $reinvestment_investment_amount = Subscription::where('status', 6)
                                            ->where('reinvestment_status', 1)
                                            ->where('user_id', $user_id)
                                            ->sum('initial_investment');  

        $start_date2 = date('Y-m-d');
        $upcomeing_query = Subscription::where('status', 3)
                                        ->whereNotNull('reinvestment_parant_id')
                                        ->where('user_id', $user_id)
                                        ->where('commence_date', '>=', $start_date2)
                                        ->get();   

        $upcomeing_investment_count = $upcomeing_query->count();
        ...
        $enable_subscription = Subscription::with('User', 'McilFund')
                            ->where('user_id',$user_id)
                            ->where('draft',0)
                            ->where('is_first',1)
        ...
        return view('individual.dashboard', compact('investment_count', 'total_dividends', 'investment_amount', 'subscription2', 'subscription3', 'flash_msg', 'subscription4', 'initial_obj', 'maturity_exps', 'redemption_investment_count', "redemption_investment_amount", "upcomeing_investment_count", "redemption_upcomeing_count", 'above_investment_count', 'reinvest_investment_count', 'reinvestment_investment_amount', 'active_investments', 'enable_subscription'));
    }
    ...
    //*****************************************************************************//
        ...
        return view('individual.subscription.subscriptionIndex', ['subscriptions' => $subscriptions, 'subscriptionsDraft' => $subscriptionsDraft, 'enable_subscription' => $enable_subscription, 'q' => $q]);

        // return view('individual.subscription.subscriptionIndex', compact('subscriptions', 'subscriptionsDraft', 'enable_subscription'));
    }
    ...
    public function subscriptionInitialCreate(Request $request){
        ...
        $mobile_prefix = $userData->mobile_prefix ? $userData->mobilePrefix->calling_code : '';
        ...
        // return $userData;
        return view('individual.initialCreate', ['edit' => $edit, 'countries'=> $countries, 'mobile_prefix'=> $mobile_prefix, 'phone_prefix'=> $phone_prefix, 'userData'=> $userData, 'subscription' => $subscription, 'additional' => $additional, 'fund_types' => $fund_types]);
    }
    ...
    public function subscriptionInitialSave(Request $request)
    {
        ...
        $user = User::with('countryAs','stateAs', 'individual')->findOrFail($user_id);
        ...
        if($subscription){
            ...
            $actionLog['action'] = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_id." Initial investment Created";
    ...
    public function subscriptionAdditionalCreate(Request $request)
    {
        ...
        return view('individual.additionalCreate', $userData);
    }
    ...
    public function subscriptionAdditionalSave(Request $request)
    {
        ...
        $noti_link = "/subscription/view/".$subscription->id;
        $investment_no = $subscription->refreance_id;
        $noti_message = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no ."" ."New Additional investment";
        ...
        $actionLog['is_doc_enable'] = 1; 
        $actionLog['model'] = "Subscriptions"; 
        $actionLog['action'] = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no."Additional Investment Created.";
        $actionLog['clicked_from'] = $_SERVER['HTTP_REFERER'];
        ...
        return \Redirect::to('/individual/subscriptions')->with('success', 'The Additional Subscription save successfully.');
    }

    public function subscriptionView($id)
    {
        $userId = Auth::user()->id;
        $subscription = Subscription::with(['User.stateAs', 'User.countryAs', 'User.mobilePrefix', 'SsAttachments', 'Individual.IndiResidence', 'Individual.IndiNationality', 'Company', 'Payments', 'McilFund', 'SubscriptionCountry', 'SubscriptionState', 'SubscriptionJaCountry', 'SubscriptionJaState', 'SubscriptionTrState', 'SubscriptionTrCountry', 'SubscriptionJaNationality' ,'SubscriptionJaMobilePrefix', 'SubscriptionJaResidence', 'SubscriptionB1PhonePrefix', 'SubscriptionB1Country', 'SubscriptionB1State', 'SubscriptionB1Nationality', 'SubscriptionB1Residence', 'SubscriptionB2Country', 'SubscriptionB2State', 'SubscriptionB2PhonePrefix', 'SubscriptionB2Nationality', 'SubscriptionB2Residence', 'SubscriptionTd1Country', 'SubscriptionTd2Country', 'SubscriptionTd3Country', 'SubscriptionJaTrCountry', 'SubscriptionJaTrState', 'SubscriptionJaTd1Country', 'SubscriptionJaTd2Country', 'SubscriptionJaTd3Country'])
            ->whereIn('status', [0,1, 2, 3,6])
            ->where('user_id', $userId)
            ->findOrFail($id);

        $user = User::with('countryAs','stateAs','mobilePrefix','individual.IndiResidence','individual.IndiNationality')->findOrFail($userId);
        ...
        $active_investment = Subscription::with('User','Payments','McilFund')
                            ->where('status', 3)
                            ->where('reinvestment_status', 0)
                            ->where('id', $id)
                            ->where('user_id', $userId)
        ...
        return view('individual.subscription.subscriptionView', compact('subscription', 'user', 'above_investment_count', 'active_investment', 'maturity_exps'));
    ...
    public function documentBankIndex(Request $request)
    {
        ...
        return view('individual.documentBankIndex', compact('subscriptions', 'attachment'));
    }
    ...
    public function documentBankUpload(Request $request)
    {
        ...
        $subscription = Subscription::findOrFail($sub_att_id);
        $subscription->notification_doc_hidden = 1;
        $subscription->save();

        if($ss_attachments=$request->file('ss_attachments')){
            foreach ($ss_attachments as $ss) {
                ...
                $ssAttachment->subscription_id = $sub_att_id;
    ...
    {
        $user_id = Auth::user()->id;
        $subscription = Subscription::with(['User','SsAttachments', 'Individual', 'User.stateAs', 'User.countryAs', 'User.mobilePrefix', 'McilFund', 'Payments', 'SubscriptionCountry', 'SubscriptionState', 'SubscriptionJaCountry', 'SubscriptionJaState', 'SubscriptionTrState', 'SubscriptionTrCountry', 'SubscriptionJaNationality' ,'SubscriptionJaMobilePrefix', 'SubscriptionB1PhonePrefix', 'SubscriptionB1Country', 'SubscriptionB1State', 'SubscriptionB1Nationality', 'SubscriptionB2Country', 'SubscriptionB2State', 'SubscriptionB2PhonePrefix', 'SubscriptionB2Nationality', 'SubscriptionTd1Country', 'SubscriptionTd2Country', 'SubscriptionTd3Country', 'SubscriptionJaTrCountry', 'SubscriptionJaTrState', 'SubscriptionJaTd1Country', 'SubscriptionJaTd2Country', 'SubscriptionJaTd3Country'])
            ->where('id',$subscription_id)
            ->where('user_id',$user_id)
            ->first();

        $user = User::with('countryAs', 'stateAs', 'mobilePrefix' ,'individual.IndiResidence', 'individual.IndiNationality')->findOrFail($user_id);
    ...
    public function pfia($subscription_id)
    {
        ...
        $userData = array('subscription' => $subscription, 'user' => $user, 'old_address' => $subscription->old_address, 'currency_word' => $currency_word, 'old_subscription' => $old_subscription, 'quarters' => $quarters);

        if($subscription->fund_type == 2){
            $pdf = \PDF::loadView('pdf.mcil2.activePfia', $userData);
            return $pdf->stream('activePfia.pdf');
        }else if($subscription->fund_type == 3){
            $pdf = \PDF::loadView('pdf.mcil3.activePfia', $userData);
            return $pdf->stream('activePfia.pdf');
        } else {
            $pdf = \PDF::loadView('pdf.activePfia', $userData);
            return $pdf->stream('activePfia.pdf');
        }
    }
    ...
    public function settings(Request $request)
    {
        ...
        return view('individual.settings', compact('user', 'google2fa_secret', 'qr_image'));
    }
    ...
    public function changeBankDetailsUpload(Request $request)
    {
        ...
        if ($ssAttachment) {
            ...
            $actionLog['is_doc_enable'] = 1; 
            $actionLog['model'] = "Subscriptions"; 
            $actionLog['action'] = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no." Individual Bank Detail Change Request.";
            $actionLog['clicked_from'] = $_SERVER['HTTP_REFERER'];
    ...
    public function changeBeneficiaryDetailsUpload(Request $request)
    {
        $user_id = Auth::user()->id;
        $sub_att_id = $request->get('sub_att_id');
        ...
        $ssAttachment1['attachment'] = $image_name;
        $ssAttachment1['attachment_type'] = 31;
        $ssAttachment1['notification'] = 0;
        $ssAttachment1['subscription_id'] = $subscription->id;
        ...
        if ($ssAttachment) {
            ...
            sendBeneficiaryNoticeForAdmin($user, $subscription);
            sendBeneficiaryNoticeForInvestor($user, $subscription);
            ...
            if($user->id){
                ...
                $noti_receiver_user_id = 1;
                $noti_link = "/subscription/view/".$subscription->id;
                $noti_message = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no." The Beneficiary Change Request";
            ...
            $actionLog['is_doc_enable'] = 1; 
            $actionLog['model'] = "Subscriptions"; 
            $actionLog['action'] = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no." Requested Beneficiary Change Details.";
            $actionLog['clicked_from'] = $_SERVER['HTTP_REFERER'];
            ...
            $subscriptionData->beneficiary_status = 1;

            if ($beneficiary_status == 1) {
               $subscriptionData->b1_status = $beneficiary_status;
            } else {
                $subscriptionData->b2_status = $beneficiary_status;
            }

            $subscriptionData->save();

            return response()->json(['data' => "done", 'error' => true, 'msg' => "The beneficiary form has been saved"], 200); 
        }

        return response()->json(['data' => "null", 'error' => false, 'msg' => "The beneficiary form could not be saved. Please, try again."], 200); 
    ...
    public function redemptionUpload(Request $request)
    {
        ...
        if ($updated) {
            ...
            $actionLog['is_doc_enable'] = 1; 
            $actionLog['model'] = "SsAttachments"; 
            $actionLog['action'] = $msg_fund_type."-".$user->first_name.$user->last_name." - ".$investment_no." The Redemption Request Documents Uploaded.";
            $actionLog['clicked_from'] = $_SERVER['HTTP_REFERER'];
    ...
    public function notificationIndex(Request $request)
    {
        ...
        return view('individual.messages.index', compact('emails')); 
    }
    ...
    public function notificationView(Request $request, $id)
    {
        ...
        return view('individual.messages.view', compact('email')); 
    }
    ...
    public function fmsgIndex(Request $request)
    {
        ...
        return view('individual.flashmsgs.index', compact('flash_msgs'));
    }

    public function fmsgView(Request $request, $id)
    {
        $fmsg = Flashmsg::findOrFail($id);
        return view('individual.flashmsgs.view', compact('fmsg'));
    }
