...
<body>
    <div class="main_div">
        ...
        <table width="100%">
            <tr>
                <td rowspan="5" width="20%" align="left"><img src="<?= SITE_URL; ?>img/site/mcil-logo.png" width="110px;" height="110px"></td>
                <td align="left">
                    <h1><strong>MCIL INTL SERIES 2 LTD (Company No. LL17226)</strong></h1><br>
        ...
        <img src="<?= SITE_URL; ?>img/media/hr.png" width="100%" height="15px">
		...
		<table width="100%">
            ...
            <tr>
                <td><h4>Address</h4></td>
                <td colspan="3"><?= $user->addr_line_1 ?>, 
                        <?= $user->addr_line_2 ?> 
                        <?= $user->city ?>  <?= $user->postcode ?> 
                        <?= $user->has('user_state') ? $user->user_state->name : '' ?>,
                        <?= $user->has('user_country') ? $user->user_country->name : '' ?>.
                </td>
            </tr>
            <tr><td><h4>Date:</h4></td><td  colspan="3"><?php 
			if(!empty($subscription->pfia_date)){ 
				echo $subscription->pfia_date->format('d-F-Y'); 
			}else{ 
				echo $subscription->created->format("d-F-Y");
			} 
		?></td></tr>
        ...
        <h4>Dear : <?= $user->last_name ?></h4>
		<h4><u><strong>RE: MCIL INTL SERIES 2 FUND INVESTMENT AGREEMENT (PFIA)</strong></u></h4>
		<p>Thank you for your investment into MCIL INTL SERIES 2 FUND. We appreciate your confidence in us and we hope that our investment plan will be able to serve your financial needs well.</p>
		...
		<table width="100%" class="bankdetails" cellspacing="0" cellpadding="0">
			<tbody>		
				<tr>
					...
					<td width="60%"><p><?= $user->last_name ?> </p></td>
				</tr>
				...
				<tr>
					...
					<td><p><?= $user->has('company') ? $user->company->user_country_compnay->name : '' ?></p></td>
				</tr>

				<tr>
					<td><p>Address :</p></td>
					<td><p><?= $user->addr_line_1 ?>  
                        <?= $user->addr_line_2 ?>  
                        <?= $user->city ?> <?= $user->postcode ?>  
                        <?= $user->has('user_state') ? $user->user_state->name : '' ?> 
                        <?= $user->has('user_country') ? $user->user_country->name : '' ?>.</p></td>
				</tr>
					...
					<td><p>
                        <?php
                            if(!empty($subscription->commence_date)){
                                ...
                                echo $subscription->commence_date->format("Y-M-d");
                            }else{
                                echo " ";
                            } ?></p>
                    </td>
				...
				<tr>
					...
					<td><p><?php
                        if(!empty($subscription->maturity_date)){
                            echo $subscription->maturity_date->format("Y-M-d");
                        }else{
                            echo " ";
                        } ?></p></td>
				</tr>
                                ...
                                if(!empty($subscription->maturity_date)){
                                    ...
                                    foreach ($quarters as $key => $value) {
                                        ...
                                        if ($i == 1) {
                                            ...
                                            $ts1 = strtotime($subscription->commence_date->format('Y-m-d'));
		...
		<p>Thank you</p>
		<p>Warmest regards,</p>
		<p><img src="<?= SITE_URL; ?>img/supporting-docs-samples/director_signature.png"></p>
