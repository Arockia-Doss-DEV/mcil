    ...
    <div class="profile-body">
        <div class="body-wrapper">
            <ul class="nav navSideNewUl">
                <li class="nav-item">
                    <a class="nav-link" href="{{ url('/individual/notifications') }}">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ url('/individual/settings') }}">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('logout') }}" onclick="event.preventDefault();
                                                     document.getElementById('investor-form').submit();">
                        <i data-feather="log-out" class="menu-icon"></i>
                        <span class="menu-title">Logout </span>
                    </a>

                    <form id="investor-form" action="{{ route('logout') }}" method="POST" class="d-none">
                        @csrf
                    </form>
                </li>
            </ul>
...
<nav class="navbar fixed-top">
    <div class="navbar-menu-container d-flex align-items-center justify-content-between">
        ...
        <div class="text-center navbar-brand-container align-items-center justify-content-center">
            <a class="brand-logo" href="{{ url('/individual/dashboard') }}">
        ...
        <ul class="navbar-nav">
            <li class="nav-item custom-mobile-navView">
                <a class="nav-link {{ $noti_count != 0 ? 'count-highlighter' : '' }}" href="{{ url('/individual/notifications') }}">
            ...
            <li class="nav-item d-none d-sm-block admin-dropdown admin-profile-dropdown">
                <select class="selectpicker" id="investorLogout">
                    <option value="0"> {{ auth()->user()->first_name }}</option>
                    <option value="1">Logout</option>
                </select>
            </li>

            {{-- <li class="nav-item nav-profile-name new-custom-dropdown cursor-pointer">
                <div class="dropdown nav-link ">
                    <span class="dropdown-toggle" href="#" id="dropdownMenuButton" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false">
                        {{ auth()->user()->first_name }} <i data-feather="chevron-down"></i>
                    </span>
                    <div class="dropdown-menu new-custom-dropdown-menu"
                        aria-labelledby="dropdownMenuButton">
                        <a class="dropdown-item" href="{{ route('logout') }}" onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();">
                            <i data-feather="log-out" class="new-custom-dropdown-icon"></i> Logout</a>
...
<nav class="navbar-container flex-row" id="navbar">
    <div class="primary">
        ...
        <div class="nav-wrapper">
            <ul class="nav">
                <li class="nav-item {{ (request()->is('individual/dashboard')) ? 'active' : '' }}">
                    <a class="nav-link" href="{{ url('/individual/dashboard') }}">
                        <i data-feather="home" class="menu-icon"></i>
                        <span class="menu-title">Dashboard </span>
                    </a>
                </li>

                <li class="nav-item {{ (request()->is('individual/myprofile')) ? 'active' : '' }}">
                    <a class="nav-link" href="{{ url('/individual/myprofile') }}">
               ...
               <?php if(!empty($globalSubscription)): ?>
                    <?php if($globalSubscription->notification_doc == 1): ?>
                        ...
                        <li class="nav-item {{ (request()->is('individual/documentBankIndex')) ? 'active' : '' }}">
                            <a class="nav-link" href="{{ url('/individual/documentBankIndex') }}">
                ...
                <li class="nav-item {{ (request()->is('individual/subscriptions')) ? 'active' : '' }}">
                    <a class="nav-link" href="{{ url('/individual/subscriptions') }}">
                        <i data-feather="dollar-sign" class="menu-icon"></i>
                        <span class="menu-title">My Investment </span>
                    </a>
                </li>
                <li class="nav-item {{ (request()->is('individual/newsletters')) ? 'active' : '' }}">
                    <a class="nav-link" href="{{ url('/individual/newsletters') }}">
                        <i data-feather="mail" class="menu-icon"></i>
                        <span class="menu-title">Newsletters </span>
                    </a>
                </li>
                <li class="nav-item {{ (request()->is('individual/fmsgIndex')) ? 'active' : '' }}">
                    <a class="nav-link" href="{{ url('/individual/fmsgIndex') }}">
