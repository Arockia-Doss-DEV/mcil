...
@section('title', 'Update Investment')
                ...
                <div class="pageheader">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb no-bg custom-breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Initial Investment</li>
                        </ol>
                    </nav>
                </div>
                <div class="d-flex">
                        ...
                        <div class="card-header">
                            <h5 class="card-title">
                                <img src="{{ asset('assets/images/icons/pencil-icon.png') }}"
                                    alt="edit-icon" class="mr-2" />Update Initial Investment</h5>
                        </div>
                        <div class="card-body">
                            ...
                            {{ Form::open(['route' => ['individualSubscriptionUpdate',['userId' => $user->id, 'subId' => $subscription->id]], 'method' => 'POST', 'id'=>'subscriptionform', 'class' => 'multisteps-form__form', 'data-parsley-validate'=>'data-parsley-validate', 'autocomplete'=>"off",'enctype'=>'multipart/form-data']) }}
                                ...
                                <div class="multisteps-form__panel  p-4 rounded bg-white js-active" data-animation="scaleIn">
                                    @include("admin.subscriptionElements.individual.subscription")
                                </div>

                                <div class="multisteps-form__panel  p-4 rounded bg-white" data-animation="scaleIn">
                                    @include("admin.subscriptionElements.individual.declaration")
                                </div>

                                <!--single form panel-->
                                <div class="multisteps-form__panel  p-4 rounded bg-white" data-animation="scaleIn">
                                    @include("admin.subscriptionElements.individual.documents")
                                </div>
                                
                                <!--single form panel-->
                                <div class="multisteps-form__panel  p-4 rounded bg-white" data-animation="scaleIn">
                                    @include("admin.subscriptionElements.individual.reviews")
                                </div>
...
<script type="text/javascript">  
    ...
    function saveSubscriptionDraft(){
        ...
        if ($('#subscriptionform').parsley().validate('block1')) {
            ...
            axios.post(SITE_URL+'subscription/initialSubscriptionEditDraft',formData,{
                    headers: {
                        'Content-Type': 'multipart/form-data',
                        'X-CSRF-Token': csrfToken}}
            ).then(function(response){
                ...
                if(item.error === 0){
                    ...
                    $("#review_signed_application").attr("href", SITE_URL+'subscription/signedApplication/'+item.data.id);
                    $("#link_declaration_of_source").attr("href", SITE_URL+'subscription/sourceDeclaration/'+item.data.id); 
                }
            });
        }
    }
